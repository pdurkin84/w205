...
print_help_and_exit()
{
	echo -e "$(tput bold)Usage:$(tput rmso): $THIS_SCRIPT [-h] [-q] [-v] [-s] [-n] [-L] [-l logfile] [-i inputdirectory] [-d destinationdirectory] [-o outputScript]"
	...
	echo -e "\t\t -L\t\t At the end load the SQL Script into hive"
...
{
	while getopts h?qvsnLl:i:d:o: option
	do
		case $option in
			q)	QUIET="Y"
				;;
...
create_ddl_sql_from_header ()
{
	...
	# also turns out there are fileds with % symbols in them
...
################################################################################
#	Function:		check_output_sql_file
#	Description:	Verifies that the file can be created and if it exists backs
#					it up first before truncating
################################################################################
...
check_output_sql_file ()
{
	...
	then
		...
		echo -e "-- $(date): file automatically generated by $THIS_SCRIPT" > $OUTPUT_SQL_SCRIPT	# empty it
	fi
}

################################################################################
#	Function:		main
#	Description:	Controls the whole script
################################################################################
...
main ()
{
	...
	if [[ $LOAD_HIVE_SCHEMA == "Y" ]]
	then
		log_msg "Loading $OUTPUT_SQL_SCRIPT into hive"
		hive -f $OUTPUT_SQL_SCRIPT
	fi
}
